...
class Liste:
    ...
    def __str__(self) -> str:
        return (f"Liste {self.name}, contenant {len(self.items)} "
                f"élément{'s' if len(self.items) > 1 else ''}.")

    def add_to_list(self, item):
        """Adds to items an elemnt chosen by the user."""
        self.items.append(item.lower())
        print(f"L'élément {item.upper()} a été ajouté à la liste {self.name}.")
        
    def remove_from_list(self, item):
        """Remove from items an elemnt chosen by the user,
